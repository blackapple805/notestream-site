...
const colorMap = {
  ...
  indigo: {
    ...
    button: "bg-indigo-500/20 hover:bg-indigo-500/30 text-accent-indigo",
  },
  sky: {
    bg: "bg-sky-500/10",
...
<header className="pt-2 px-1">
  ...
  {/* Upgrade Banner */}
  <GlassCard className="border-indigo-500/25 bg-gradient-to-br from-indigo-500/5 to-purple-500/5">
    <div className="flex flex-col sm:flex-row gap-4 sm:items-center sm:justify-between">
      <div className="flex items-start gap-3">
        <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center flex-shrink-0">
          <Lightning size={24} weight="fill" className="text-theme-primary" />
        </div>
        <div>
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-indigo-500 to-purple-600 text-theme-primary font-medium text-sm shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 transition active:scale-95"
      >
      ...
      {/* Pro Features Grid */}
      <div>
        <h3 className="text-sm font-semibold text-[var(--text-secondary)] mb-3 px-1">Pro Features</h3>
            ...
            return (
              ...
              >
                ...
                <div className="absolute top-3 right-3">
                  <div className="h-6 w-6 rounded-full bg-theme-elevated/30 flex items-center justify-center">
                ...
                {feature.demo ? (
                  ...
                  <span className="text-xs text-theme-muted">Requires Pro</span>
                )}
              </motion.div>
            );
          })}
      ...
      <AnimatePresence>
        {showPricing && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[999] bg-theme-elevated/70 backdrop-blur-md overflow-y-auto"
          >
            <div className="min-h-full px-4 py-8">
              <div className="max-w-4xl mx-auto">
                ...
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-2xl font-bold text-theme-primary">Choose Your Plan</h2>
                    <p className="text-sm text-theme-muted mt-1">Start free, upgrade when you're ready.</p>
                  </div>
                  <button
                    onClick={() => setShowPricing(false)}
                    className="h-10 w-10 rounded-xl bg-theme-button flex items-center justify-center text-theme-muted hover:text-theme-primary transition"
                  >
                    <FiX size={20} />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {pricingPlans.map((plan) => (
                    <div
                      key={plan.name}
                      className={`relative bg-theme-primary border rounded-2xl p-5 ${
                        plan.popular
                    ...
                    >
                      {plan.popular && (
                        <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-gradient-to-r from-indigo-500 to-purple-600 text-[10px] font-semibold text-theme-primary">
                          MOST POPULAR
                        </div>
                      )}

                      <h3 className="text-lg font-semibold text-theme-primary mb-1">{plan.name}</h3>
                      ...
                      <button
                        ...
                        className={`w-full py-2.5 rounded-xl font-medium text-sm transition ${
                          plan.current
                            ? "bg-theme-button text-theme-muted cursor-default"
                            : plan.popular
                            ? "bg-gradient-to-r from-indigo-500 to-purple-600 text-theme-primary hover:opacity-90"
                            : "bg-theme-button text-theme-primary hover:bg-[#262631]"
                        }`}
                      >
      ...
      <AnimatePresence>
        {showDemo && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[999] bg-theme-elevated/70 backdrop-blur-md flex items-center justify-center px-4"
          >
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-theme-primary border border-[var(--border-secondary)] rounded-2xl p-6 max-w-md w-full"
            >
              ...
              <p className="text-sm text-theme-muted mb-4">{showDemo.desc}</p>

              {/* Demo Preview */}
              <div className="bg-[var(--bg-input)] border border-[var(--border-secondary)] rounded-xl p-4 mb-4">
                {showDemo.id === "voice" && (
                  <div className="text-center py-6">
                    ...
                    <p className="text-sm text-[var(--text-secondary)]">Tap to record a voice note</p>
                    <p className="text-xs text-theme-muted mt-1">AI will transcribe and format it</p>
                  </div>
                )}
                {showDemo.id === "collab" && (
                  <div className="text-center py-6">
                    <div className="flex -space-x-2 justify-center mb-3">
                      {["bg-indigo-500", "bg-emerald-500", "bg-amber-500"].map((bg, i) => (
                        <div key={i} className={`w-10 h-10 rounded-full ${bg} border-2 border-[#0d0d14] flex items-center justify-center text-theme-primary text-sm font-medium`}>
                          {["A", "B", "C"][i]}
                        </div>
                      ))}
                    </div>
                    <p className="text-sm text-[var(--text-secondary)]">Invite team members</p>
                    <p className="text-xs text-theme-muted mt-1">Collaborate in real-time</p>
                  </div>
                )}
                {showDemo.id === "export" && (
                  <div className="text-center py-6">
                    <div className="flex gap-2 justify-center mb-3">
                      {["PDF", "DOCX", "MD"].map((format) => (
                        <div key={format} className="px-3 py-1.5 rounded-lg bg-theme-button border border-[var(--border-secondary)] text-xs text-[var(--text-secondary)]">
                          {format}
                        </div>
                      ))}
                    </div>
                    <p className="text-sm text-[var(--text-secondary)]">Export to any format</p>
                    <p className="text-xs text-theme-muted mt-1">Beautiful, professional reports</p>
                  </div>
                )}
              </div>
              ...
              <button
                ...
                className="w-full py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-theme-primary font-medium text-sm flex items-center justify-center gap-2"
              >
                <Crown size={16} weight="fill" />
                Unlock with Pro
              </button>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        ...
        <span className={`text-xs font-medium ${isLow ? "text-amber-400" : "text-[var(--text-secondary)]"}`}>
          {used}/{max}
        </span>
      </div>
      <div className="h-1.5 bg-theme-button rounded-full overflow-hidden">
        <motion.div
          initial={{ width: 0 }}
