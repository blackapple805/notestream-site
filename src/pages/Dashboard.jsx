...
import { House, MagnifyingGlass, Brain, Sparkle, Note, Bell, CalendarBlank } from "phosphor-react";
...
----------------------------------------- */
const sampleNotes = [
  { id: 1, title: "Team Meeting Notes", body: "Discuss deadline for Q2 deliverables. Remind John about the budget review. TODO: send meeting recap.", updated: new Date().toISOString(), favorite: true, tag: "Work" },
  { id: 2, title: "Project Roadmap Ideas", body: "Urgent: finalize feature list before Friday", updated: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString(), favorite: false, tag: "Ideas" },
  { id: 3, title: "Research Summary", body: "Meeting with design team tomorrow. Action item: prepare wireframes", updated: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(), favorite: true, tag: "Study" },
  { id: 4, title: "Client Call Prep", body: "Remember to follow up on proposal. Due date: next Monday", updated: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(), favorite: false, tag: "Work" },
];
...
export default function Dashboard() {
  ...
  // Parse notifications from sample notes on mount and when settings change
  useEffect(() => {
  ...
  // Generate digest when requested
  useEffect(() => {
    ...
    <div className="w-full space-y-6 sm:space-y-8 pb-[calc(var(--mobile-nav-height)+24px)]">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          {/* Notification Bell - Only show if smart notifications enabled */}
          {settings.smartNotifications && notifications.length > 0 && (
            <button
              onClick={() => setShowNotifications(true)}
              className="relative p-2 rounded-xl transition hover:bg-white/5"
              style={{ backgroundColor: 'var(--bg-tertiary)' }}
            >
              <Bell size={22} weight="duotone" className="text-indigo-400" />
      ...
      {/* Smart Notifications Banner - Collapsed view */}
      <AnimatePresence>
        {settings.smartNotifications && notifications.length > 0 && (
          <motion.div
            ...
            <GlassCard className="border-amber-500/30 bg-gradient-to-br from-amber-500/5 to-orange-500/5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-theme-primary">Smart Notifications</h3>
      ...
      {/* Weekly Digest Card - Only show if enabled */}
      <AnimatePresence>
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-xl bg-purple-500/20 border border-purple-500/30 flex items-center justify-center">
                    <CalendarBlank className="text-purple-400" size={18} weight="duotone" />
                  </div>
                  <div>
                    <h3 className="text-sm font-semibold text-theme-primary">Weekly Digest</h3>
                    <p className="text-xs text-theme-muted">{digest.period.start} - {digest.period.end}</p>
                  </div>
                </div>
                <button
          ...
          >
            ...
            >
              <div className="p-4 border-b flex items-center justify-between" style={{ borderColor: 'var(--border-secondary)' }}>
                <div className="flex items-center gap-2">
                  <FiBell className="text-amber-400" size={18} />
                  <h2 className="text-lg font-semibold text-theme-primary">Smart Notifications</h2>
                  ...
                  notifications.map((notif) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-theme-muted mt-1">From: {notif.noteTitle}</p>
                      </div>
                ...
                >
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-theme-secondary font-medium">{digest.insights.mostActiveDay}</span>
                    </div>
                    <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-theme-secondary font-medium">{digest.stats.totalItems}</span>
                    </div>
                  </div>
                </div>
